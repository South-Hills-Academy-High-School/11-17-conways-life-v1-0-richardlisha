...
function updateGrid () {
    ...
    for (let row = 0; row <= 11; row++) {
        ...
        for (let column = 0; column <= 15; column++) {
            currentCe11A1iveorDead = grid[row][column]
            currentCe11Neighbors = countNeighbors(row, column)
            if (currentCe11A1iveorDead == 1 && currentCe11Neighbors < 2) {
                nextGrid[row].push(0)
            } else if (currentCe11A1iveorDead == 1 && currentCe11Neighbors > 3) {
                nextGrid[row].push(0)
            } else if (currentCe11A1iveorDead == 0 && currentCe11Neighbors == 3) {
                nextGrid[row].push(1)
            } else {
                nextGrid[row].push(currentCe11A1iveorDead)
            }
        }
    }
    grid = nextGrid
...
function countNeighborsWrapRight (currentRow: number, currentCol: number) {
    ...
    neighborCount += grid[currentRow - 1][15 + 0]
    neighborCount += grid[currentRow - 1][15 - 1]
    neighborCount += grid[currentRow - 0][15 - 1]
    neighborCount += grid[currentRow + 1][15 - 1]
    neighborCount += grid[currentRow + 1][15 + 0]
    neighborCount += copyLeft(currentRow + 1)
...
let currentCe11Neighbors = 0
let currentCe11A1iveorDead = 0
let nextGrid: number[][] = []
let neighborCount = 0
let autoFlag = 0
